...
CommandResponse makeCalResponse(const char* fmt = nullptr, ...)
{
    ...
    size_t off = static_cast<size_t>(
        std::snprintf(rsp.body.data(), rsp.body.size(), R"({"status":"ok","data":{)"));
    if (fmt)
    {
        va_list args;
        va_start(args, fmt);
        int n = std::vsnprintf(rsp.body.data() + off, rsp.body.size() - off, fmt, args);
        va_end(args);
        if (n > 0)
            off += static_cast<size_t>(n);
    }
    off += static_cast<size_t>(std::snprintf(rsp.body.data() + off, rsp.body.size() - off, "}"));
...
std::expected<CommandResponse, domain::AppError> handleGetCalibration(ReadCalCb readCal)
{
    auto cal = readCal();
    if (!cal)
    {
    ...
    off += static_cast<size_t>(std::snprintf(
        rsp.body.data() + off, rsp.body.size() - off,
        R"("steps_per_ml":%.1f,)"
        R"("nominal_vol":%.2f,"speed_coeff":%.5f,)"
        R"("min_freq":%u,"max_freq":%u,)"
        R"("is_default":true})",
        static_cast<double>(cal->stepsPerMl), static_cast<double>(cal->nominalVolumeMl),
        static_cast<double>(cal->speedCoeff), static_cast<unsigned>(cal->minFreqHz),
...
handleCalcVolume(std::optional<domain::Steps> steps, std::optional<float> massG,
                 std::optional<float> temperature, std::optional<float> pressure, ReadCalCb readCal)
{
    (void)steps;
    if (!massG || *massG <= 0.0f)
    {
    ...
    float newSpm = domain::calculateNewStepsPerMl(cal->stepsPerMl, cal->nominalVolumeMl, actualVol);
    float relError = (actualVol - cal->nominalVolumeMl) / cal->nominalVolumeMl * 100.0f;

    gPendingCal.stepsPerMl = newSpm;
    gPendingCal.nominalVolumeMl = actualVol;

    return makeCalResponse(R"("z_factor":%.6f,"actual_volume_ml":%.4f,)"
...
handleCalcSpeed(const float* frequencies, const float* speeds, size_t count, ReadCalCb readCal)
{
    ...
    return makeCalResponse(R"("k":%.6f,"r_squared":%.4f,"min_freq":%u,"max_freq":%u)",
                           static_cast<double>(result.k), static_cast<double>(result.rSquared),
                      ...
                      WriteCalCb writeCal)
{
    ...
    if (stepsPerMl)
        cal.stepsPerMl = *stepsPerMl;
    if (nomVolume)
        cal.nominalVolumeMl = *nomVolume;
    ...
    gPendingCal = cal;
    return makeCalResponse(R"("stepsPerMl":%.1f,"nominalVolume":%.1f)",
                           static_cast<double>(cal.stepsPerMl),
                           static_cast<double>(cal.nominalVolumeMl));
}

std::expected<CommandResponse, domain::AppError> handleResetCalibration(WriteCalCb writeCal)
{
...
handleRunCalibration(const float* freqs, size_t freqsCount, float speedMlMin)
{
    ...
    cmd.startCalSpeedSeq.fillSpeedMlMin =
        (speedMlMin > 0.0f) ? speedMlMin : config::FILL_DEFAULT_SPEED_ML_MIN;
    for (size_t i = 0; i < count; ++i)
    {
        cmd.startCalSpeedSeq.freqs[i] =
            (freqs[i] > 0.0f) ? static_cast<uint16_t>(std::lround(freqs[i])) : 0;
    }
...
std::expected<CommandResponse, domain::AppError> handleGetCalResult(ReadCalCb readCal)
{
    ...
    domain::SmResult result =
        resultOpt.value_or(domain::SmResult{domain::SmResult::Type::None, 0, 0.0f, {}, 0});
    CommandResponse rsp;
    rsp.kind = ResponseKind::Single;
    size_t off = 0;
    ...
    {
        off = static_cast<size_t>(std::snprintf(
            rsp.body.data(), rsp.body.size(), R"({"status":"ok","data":{"result":"no_result"}})"));
        rsp.bodySize = off;
        return rsp;
    }

    off = static_cast<size_t>(
        std::snprintf(rsp.body.data(), rsp.body.size(), R"({"status":"ok","data":{"result":"ok")"));
    ...
    case domain::SmResult::Type::CalSpeedSeqComplete: {
        ...
        for (int i = 0; i < result.resultCount && i < 3; ++i)
        {
            ...
            off +=
                static_cast<size_t>(std::snprintf(rsp.body.data() + off, rsp.body.size() - off,
