...
std::expected<CommandResponse, domain::AppError> handleEmpty()
{
    ...
    if (steps < config::MIN_STEPS_THRESHOLD)
        steps = static_cast<int32_t>(std::lround(cal.nominalVolumeMl * cal.stepsPerMl));
...
std::expected<CommandResponse, domain::AppError> handleRinse(std::optional<uint32_t> cycles)
{
    if (!cycles || *cycles == 0)
    {
        return makeErrorResponse("invalid_params");
...
std::expected<CommandResponse, domain::AppError> handleCalRun(std::optional<std::string_view> mode,
                                                              ...
                                                              std::optional<float> speedMlMin)
{
    ...
    uint16_t freq = (freqHz && *freqHz > 0) ? static_cast<uint16_t>(std::lround(*freqHz)) : 0;
    float speed = speedMlMin.value_or(0.0f);

    char modeBuf[8];
    modeBuf[0] = '\0';
    size_t modeLen = mode->size();
    if (modeLen > sizeof(modeBuf) - 1)
        modeLen = sizeof(modeBuf) - 1;
    std::memcpy(modeBuf, mode->data(), modeLen);
    modeBuf[modeLen] = '\0';

    auto plan = domain::sm::planCalRun(modeBuf, speed, freq, static_cast<float>(cal.maxFreqHz),
...
std::expected<CommandResponse, domain::AppError> handleEmergencyStop()
{
    ...
    return makeSingleResponse(std::string_view(R"({"status":"emergency_stopped"})"),
...
std::expected<CommandResponse, domain::AppError> handleMoveSteps(std::optional<domain::Steps> steps)
{
    if (!steps)
    {
        return makeErrorResponse("invalid_params");
...
std::expected<CommandResponse, domain::AppError> handleSetSpeed(std::optional<uint32_t> speedHz)
{
    if (!speedHz)
    {
        return makeErrorResponse("invalid_params");
...
std::expected<CommandResponse, domain::AppError> handleSetAccel(std::optional<uint32_t> accelSteps)
{
    if (!accelSteps)
    {
        return makeErrorResponse("invalid_params");
