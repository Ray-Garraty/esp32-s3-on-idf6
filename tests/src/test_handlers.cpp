...
{
    auto rsp = burette_ops::handleGetStatus(BuretteState::Idle, 2530, ValvePosition::Input, 150.0f,
                                            Direction::LiqIn, 1000, 500, 50.0f);
    REQUIRE(rsp);
    REQUIRE(rsp->kind == ResponseKind::Single);
    std::string_view sv(rsp->body.data(), rsp->bodySize);
    REQUIRE(sv.find("status") != std::string_view::npos);
...
static std::expected<void, ResourceError> testSaveCalCb(const CalibrationData& cal)
{
    gCalSaveCalled = true;
    gSavedCal = cal;
    return {};
...
static std::expected<void, ResourceError> testResetCalCb(const CalibrationData& cal)
{
...
{
    auto rsp =
        burette_cal::handleCalcVolume(std::nullopt, 10.0f, std::nullopt, std::nullopt, stubCalRead);
    REQUIRE(rsp);
    REQUIRE(rsp->kind == ResponseKind::Single);
    std::string_view sv(rsp->body.data(), rsp->bodySize);
...
{
    auto rsp = burette_cal::handleCalcVolume(std::nullopt, std::nullopt, std::nullopt, std::nullopt,
                                             stubCalRead);
    REQUIRE(rsp);
    REQUIRE(rsp->kind == ResponseKind::Error);
    std::string_view sv(rsp->body.data(), rsp->bodySize);
...
{
    auto rsp =
        burette_cal::handleCalcVolume(std::nullopt, 0.0f, std::nullopt, std::nullopt, stubCalRead);
    REQUIRE(rsp);
    REQUIRE(rsp->kind == ResponseKind::Error);
}
...
{
    auto rsp = burette_cal::handleCalcVolume(std::nullopt, 10.0f, 20.0f, 80.0f, stubCalRead);
    REQUIRE(rsp);
    REQUIRE(rsp->kind == ResponseKind::Single);
    std::string_view sv(rsp->body.data(), rsp->bodySize);
    REQUIRE(sv.find("z_factor") != std::string_view::npos);
...
static uint16_t mockSampleRead()
{
    return s_mockSampleVal;
}
...
TEST_CASE("handler: adc.cal full flow", "[handlers][sensors][adc.cal]")
{
    ...
    REQUIRE(rsp2);
    REQUIRE(std::string_view(rsp2->body.data(), rsp2->bodySize).find("\"point\":2") !=
            std::string_view::npos);

    s_mockSampleVal = 2200;
    auto rsp3 = sensors::handleAdcCalMeasure(350.0f, mockSampleRead, mockAdcCalWrite);
    REQUIRE(rsp3);
    REQUIRE(std::string_view(rsp3->body.data(), rsp3->bodySize).find("\"point\":3") !=
            std::string_view::npos);

    s_mockSampleVal = 2500;
    auto rsp4 = sensors::handleAdcCalMeasure(-350.0f, mockSampleRead, mockAdcCalWrite);
    REQUIRE(rsp4);
    REQUIRE(std::string_view(rsp4->body.data(), rsp4->bodySize).find("\"point\":4") !=
            std::string_view::npos);
    ...
    REQUIRE(
        std::string_view(resetRsp2->body.data(), resetRsp2->bodySize).find("\"status\":\"ok\"") !=
        std::string_view::npos);
    ...
    REQUIRE(
        std::string_view(rspAfterReset->body.data(), rspAfterReset->bodySize).find("\"point\":0") !=
        std::string_view::npos);
}
...
{
    auto rsp = system::handleGetStatus(BuretteState::Dosing, 0, ValvePosition::Output, 100.0f,
                                       Direction::LiqOut, 2000, 400, 25.0f);
    REQUIRE(rsp);
    REQUIRE(rsp->kind == ResponseKind::Single);
    std::string_view sv(rsp->body.data(), rsp->bodySize);
