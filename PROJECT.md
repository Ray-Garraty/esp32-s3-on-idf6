# Technical Specification: EcoTiter Firmware (Rust/ESP-IDF v6)
...
| Step frequency | 30–3000 Hz (period 33333–333 µs) |
...
|---|---|---|---|
| TMC2209 STEP | 25 | TxChannelDriver (RMT) | Pulse train, 1 µs HIGH |
...
| DS18B20 | 33 | OneWire bitbang | 4.7kΩ pull-up |
...
Step pulses are generated by the ESP32 RMT peripheral (channel 0, TX mode, 1 MHz resolution). Each step = 2 RMT symbols: 1 µs HIGH pulse + (interval − 1) µs LOW. The RMT hardware generates the pulse train with zero CPU load per pulse.
...
- `compute_ramp()` yields a `Vec<u32>` of per-step intervals in µs
...
| `AdcBuf` | 64 × u16 | ADC rolling average |
...
| `cal` | `a_x1000` | ADC calibration slope ×1000 |
