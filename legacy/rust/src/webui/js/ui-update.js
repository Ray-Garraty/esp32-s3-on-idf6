/**
 * @file ui-update.js
 * @brief DOM update logic — pure data selectors + side-effect renderers
 */
...
const toggleSgThresholdEdit = async () => {
  ...
  if (!APP_STATE.ui.sgEditMode) {
    ...
    if (isNaN(val) || val < 0 || val > CONFIG.SG_THRESHOLD_MAX) { alert(`Threshold: 0–${CONFIG.SG_THRESHOLD_MAX}`); return; }
    const btn = document.getElementById('stepperDrv-sg-threshold-btn');
...
const updateDebugUI = (data) => {
  ...
  if (diff('usbConnected')) {
    __connUsb = cur.usbConnected;
    updateConnectionStatus();
  }
  if (diff('bleConnected')) {
    __connBle = cur.bleConnected;
    updateConnectionStatus();
  }
...
const updateUI = (data) => {
  ...
  if (data.usbSerialConnected !== undefined) {
    __connUsb = data.usbSerialConnected;
    updateConnectionStatus();
  }
  if (data.bleConnected !== undefined) {
    __connBle = data.bleConnected;
    updateConnectionStatus();
  }
  if (data.vlv) {
