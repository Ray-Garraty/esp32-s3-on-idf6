  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data.status === 'ok') {
        ...
        console.error('toggleValve: server returned error', JSON.stringify(data));
      }
    } else {
...
const loadInitialLogs = () => {
  fetch(`/api/logs?limit=${CONFIG.LOG_DEFAULT_LIMIT}`).then(r => {
    ...
    if (data.entries) {
      ...
      console.error('loadInitialLogs: unexpected response', JSON.stringify(data));
    }
