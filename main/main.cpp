...
static void logDramBeforeTask(const char* name, size_t stackSize) {
    ...
    ESP_LOGI(TAG, "Creating %s, stack=%lu, largest_free=%lu",
             name, (unsigned long)stackSize, (unsigned long)largest);
    if (largest < stackSize + 4096) {
        ESP_LOGW(TAG, "DRAM pressure! Largest free block %lu < needed %lu",
                 (unsigned long)largest, (unsigned long)(stackSize + 4096));
    }
}
...
static void ensureGpioReady() {
    ...
    puts("DBG: PHY wait start"); fflush(stdout);
    vTaskDelay(PHY_CALIBRATION_WAIT_TICKS);
    puts("DBG: PHY wait done"); fflush(stdout);
...
extern "C" void app_main(void) {
    ...
    // ====== Step 4: Serial ======
    puts("DBG: step 4 - serial"); fflush(stdout);
    ...
    puts("DBG: step 5 - RWDT init"); fflush(stdout);
    ...
    puts("DBG: step 6 - BLE init"); fflush(stdout);
    ...
    puts("DBG: step 7 - PHY wait"); fflush(stdout);
    ...
    // ====== Step 8: Motor task ======
    puts("DBG: step 8 - xTaskCreate motor"); fflush(stdout);
    ...
    puts("DBG: step 9 - RUNNING"); fflush(stdout);

    diag::FfiGuard guard(50);
    infrastructure::drivers::AdcDriver adc(
